...
This document was originally written sometime around 2002 and is based on the accumulative knowledge I required over the years from about 1997 up to 2001 having to work on a relativity low-level with Win32 bitmaps.
...
After loading the bitmap from the resource it’s time to display it in our window. As you can see in the `OnPaint` function of our program, we’ve first got to create a DC (Device Context) for our bitmap. This is because the `BitBlt` function were going to use can only copy bitmaps from one DC to another.
